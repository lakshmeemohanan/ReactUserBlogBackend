...
const signup = async (req, res, next) => {
    ...
    if(!errors.isEmpty()){
        const error = new HttpError('Invalid inputs, please check the values entered',422);
        return next(error);
    }
...
const login = async (req, res, next) => {
    ...
    try{
        ...
        const error = new HttpError('Could not log you in, please check the credentials entered!', 500);
        next(error);
    }
    if(!isValidPassword){
        const error = new HttpError('Invalid credentials, could not log you in!', 403);
